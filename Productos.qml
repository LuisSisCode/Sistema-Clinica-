...
function actualizarDesdeDataCentral() {
    ...
    var hayCambios = productosOriginales.length !== nuevosProductos.length
    if (!hayCambios) {
        for (var j = 0; j < nuevosProductos.length; j++) {
            if (JSON.stringify(productosOriginales[j]) !== JSON.stringify(nuevosProductos[j])) {
                hayCambios = true
                break
            }
        }
    ...
    if (hayCambios) {
        ...
        console.log("⏭️ No hay cambios en los productos, omitiendo actualización")
    }
